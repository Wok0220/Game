...
public class LevelSelectScreen extends JPanel implements Refreshable {
    ...
    private void initUI() {
        ...
        int levelWidth = 150;
        int levelHeight = 150;
        int startX = (Constants.WINDOW_WIDTH - levelWidth) / 2;
        int startY = (Constants.WINDOW_HEIGHT - levelHeight) / 2 - 50;

        for (int i = 0; i < levelCount; i++) {
            levelRects[i] = new Rectangle(startX, startY, levelWidth, levelHeight);
        }
    }
        ...
        for (int i = 0; i < levelRects.length; i++) {
            ...
            if (levelImage != null) {
                ...
                if (isHover) {
                    ...
                    g.drawImage(levelImage, x, y, w, h, null);
                } else {
                    g.setComposite(AlphaComposite.getInstance(AlphaComposite.SRC_OVER, 0.9f));
                    g.drawImage(levelImage, levelRects[i].x, levelRects[i].y,
            ...
            // 绘制关卡编号
            String levelNum = "第 " + (i + 1) + " 关";
            g.setFont(new Font("微软雅黑", Font.BOLD, 20));
            g.setColor(Color.WHITE);

            FontMetrics fm = g.getFontMetrics();
            int textWidth = fm.stringWidth(levelNum);
            int textX = levelRects[i].x + (levelRects[i].width - textWidth) / 2;
            int textY = levelRects[i].y + levelRects[i].height + 30;
